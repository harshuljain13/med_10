...
for i in range(0,20-nT):
    ...
    ex1s=(ex1s*i+data[i+nT]**2)/(i+1)           ##estimate of x1 squared
    ex2s=(ex2s*i+data1[i+nT]**2)/(i+1)          ##estimate of x2 squared
    ex1x2=(ex1x2*i+data[i+nT]*data1[i])/(i+1)   ##estimate of x1*x2
    ess=(ess*i+sas[i+nT]**2)/(i+1)              ##estimate of sas squared
    esdx1=(esdx1*i+data[i+nT]*sas[i])/(i+1)     ##estimate of s_delay*x1
    esdx2=(esdx2*i+data1[i+nT]*sas[i])/(i+1)    ##estimate of s_delay*x2


print ex1s,ex2s,ex1x2,esas,esdx1,esdx2,ess,esds,essd    
for i in range(20,length):
    sas.append(data[i]-rv*data1[i])
    ex1s=(ex1s*i+data[i]**2)/(i+1)
    ex2s=(ex2s*i+data1[i]**2)/(i+1)
    ess=(ess*i+sas[i]**2)/(i+1)
    esdx1=(esdx1*i+data[i]*sas[i-nT])/(i+1)
    esdx2=(esdx2*i+data1[i]*sas[i-nT])/(i+1)
    ex1x2=(ex1x2*i+data[i]*data1[i])/(i+1)
...
venous_ref=[]
for i in range(0,len(beta)-5):
    venous_ref.append(x1[20+i]- beta[i]*x2[20+i])
    
xf1,ef1,wf1=adf.nlms(x1[20:-5],venous_ref,5,1)
